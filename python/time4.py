...
def calculate_flexible_schedule(year, month, vacation_days):
    ...
    min_office_days = round(adjusted_workdays * 0.4)
    office_days = max(min_office_days, int(adjusted_workdays * 0.4))  # 确保办公室天数不低于40%
    flexible_days = adjusted_workdays - office_days

    # 确保办公室天数满足40%的要求
    if office_days / adjusted_workdays < 0.4:
        office_days = min_office_days + 1
        flexible_days = adjusted_workdays - office_days
